...
if (run === 1) {
  ParamSetFile = "../data/initial-results (3).csv" 
  paramProf = null 
} else {
...
function traj_match (data, covarTime, covarTemperature, params, times, t0, index, place) {
  ...
  function logLik (estimated) {
    ...
    // js calculation
    var simHarranged = []
    var simH = integrate(params, times,covarTime, covarTemperature)
    ...
    // read R
    // let simHarranged = []
    // var set1 = fs.readFileSync('./states.csv').toString()
    // var lines = set1.split('\n')
    // for (let i = 0; i < lines.length-1; i++) {
    //   simHarranged.push(lines[i].split(','))
    //   simHarranged[i] = Number(simHarranged[i]);console.log(simHarranged[i])
    // }

    let ar = [];
    for (let i = 0; i < simHarranged.length; i++) {
      likvalue = snippet.dObs(params[Index.obsprob], simHarranged[i], data[i][1], 1);ar.push([simH[i], data[i][1],likvalue])
